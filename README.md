...
| IP 类型 | 用途 | 是否用于 SillyTavern |
|---------|------|---------------------|
| **公网 IP** | 外部访问（你的电脑、手机 → 服务器） | ✅ **填这个** |
| **私网 IP** | 服务器内部通信（同区域其他云服务互联） | ❌ 不用 |

> 简单说：**SillyTavern 永远填公网 IP**，私网 IP 只在服务器集群内部有用。
...
- **SillyTavern 角色扮演** — 破限预设、人设扮演、剧情创作
- **创意写作** — 小说、故事、剧本、诗歌
- **纯文本问答** — 知识问答、翻译、总结
- **多账号轮询** — 个人多邮箱注册，分摊免费额度

### ❌ 不适合

- **工具调用（Function Calling）** — CaMeL 网页版不支持
- **代码执行** — 无 Code Interpreter
- **联网搜索** — 无 Web Browsing 工具
- **文件分析** — 无文件上传解析功能
- **商业用途/盈利** — 免费服务，请合理使用
